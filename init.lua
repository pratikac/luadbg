...
setb =    [[
setb | b [line file]    -- set a breakpoint to line/file|, line 0 means 'any'
...
delb =    [[
delb | del [line file]  -- removes a breakpoint|
...
run     = [[
run | r | c             -- run until next breakpoint or watch expression|
]],
...
show    = [[
show | list line file X Y   -- show X lines before and Y after line in file|
...
help    = [[
help | h [command]      -- show this list or help for command|
]],
...
local function debugger_loop(ev, vars, file, line, idx_watch)
  ...
  while true do
    ...
    elseif (command == "delb" or command == "del") then
      --{{{  delete breakpoint
    ...
    elseif (command == "run" or command == "r" or command == "c") then
